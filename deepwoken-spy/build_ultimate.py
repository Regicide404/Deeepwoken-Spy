...
game_code = r'''// =========================================================================
// --- DEEPWOKEN SPY ULTIMATE HYBRID ENGINE (ONLINE RELAY + DISCORD SEED) ---
// =========================================================================
...
function updateConnectionDot(online) {
  ...
  if (dot) {
    ...
    dot.title = online ? 'Сеть активна (0ms)' : 'Подключение...';
  }
}
...
// =========================================================================
// --- NATIVE RELAY LOGIC (HTTPS PORT 443) ---
// =========================================================================

const RELAY_URL = 'https://ntfy.sh/dwspy_room_v6_';

function postNetworkMessage(payload) {
  if (!currentRoomCode) return;
  fetch(RELAY_URL + currentRoomCode, {
    method: 'POST',
...
function pollNetworkMessages() {
  if (!currentRoomCode) return;
  fetch(RELAY_URL + currentRoomCode + '/json?poll=1')
    .then(r => r.text())
    .then(text => {
...
function startNetworkSync(code) {
  currentRoomCode = code.toUpperCase().trim();
  updateConnectionDot(true);

  if (pollTimer) clearInterval(pollTimer);
  pollNetworkMessages();
  pollTimer = setInterval(pollNetworkMessages, 600);
}

function broadcastState() {
  if (!isHost) return;
  postNetworkMessage({ type: 'STATE_UPDATE', room: roomState });
}
...
function sendEvent(eventType, extraData) {
  ...
  postNetworkMessage({ type: 'GAME_EVENT', event: payload });
}
...
function sendChatMessage(text) {
  ...
  postNetworkMessage({ type: 'CHAT_MESSAGE', msg: msgObj });
}
...
function updateLobbyUI(room) {
  ...
  if (!room.players || room.players.length === 0) {
    grid.innerHTML = '<div style="color: var(--text-muted); font-size: 0.9rem; text-align: center; width: 100%;">Синхронизация игроков...</div>';
    return;
  }
...
print("SUCCESS: Ultimate main.js updated! Size:", len(game_code))
