...
# Download Paho MQTT if not cached
...
game_js = r'''
// =========================================================================
// --- DEEPWOKEN SPY 3.0 — ULTRA HIGH RELIABILITY REALTIME ENGINE (PORT 443) ---
// =========================================================================
...
// Redundant Realtime Cloud Brokers (Standard Port 443 WSS first)
const CLOUD_BROKERS = [
  { host: 'broker.emqx.io', port: 443, path: '/mqtt', ssl: true, name: 'EMQX Cloud (Port 443 WSS)' },
  { host: 'broker.emqx.io', port: 8084, path: '/mqtt', ssl: true, name: 'EMQX Backup (Port 8084 WSS)' },
  { host: 'broker.hivemq.com', port: 8884, path: '/mqtt', ssl: true, name: 'HiveMQ Cloud (Port 8884 WSS)' }
];
...
function updateConnectionBadge(connected, brokerName) {
  ...
  if (dot) {
    ...
    dot.title = connected ? `Сеть: ${brokerName || 'Активна'}` : 'Подключение к облаку...';
  }
}
...
// =========================================================================
// --- MULTI-BROKER REALTIME FAILOVER ENGINE (PORT 443 WSS) ---
// =========================================================================
...
function connectRealtimeNetwork(roomCode, onConnectSuccess) {
  ...
  const clientId = 'dw_spy_' + myPlayerId + '_' + Math.floor(Math.random() * 10000);
  const cfg = CLOUD_BROKERS[activeBrokerIdx];

  try {
    mqttClient = new Paho.MQTT.Client(cfg.host, cfg.port, cfg.path, clientId);
    ...
    mqttClient.onConnectionLost = (responseObject) => {
      ...
      setTimeout(() => {
        if (currentRoomCode && (currentScreen === 'screen-lobby' || currentScreen === 'screen-game')) {
          activeBrokerIdx = (activeBrokerIdx + 1) % CLOUD_BROKERS.length;
    ...
    mqttClient.connect({
      ...
      onSuccess: () => {
        ...
        mqttClient.subscribe(wildcardTopic, { qos: 1 });
      ...
      onFailure: (err) => {
        isConnected = false;
        updateConnectionBadge(false);
        // Switch to next redundant broker
        activeBrokerIdx = (activeBrokerIdx + 1) % CLOUD_BROKERS.length;
        setTimeout(() => {
