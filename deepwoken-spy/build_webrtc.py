...
game_code = r'''
// =========================================================================
// --- DEEPWOKEN SPY ULTIMATE WEBRTC ENGINE (INSTANT HANDSHAKE & NAT STUN) ---
// =========================================================================
...
function initHostPeer(code) {
  ...
  myPeer.on('connection', (conn) => {
    // Immediately register connection in map
    hostConnections.set(conn.peer, conn);

    conn.on('open', () => {
      hostConnections.set(conn.peer, conn);
      // Directly send current state to newly connected guest!
      try { conn.send({ type: 'STATE_UPDATE', room: roomState }); } catch(e) {}
    });
  ...
  myPeer.on('error', (err) => {
    if (err.type === 'unavailable-id') {
      showToast('Код занят, создаем новый...');
      setTimeout(() => {
...
function initGuestPeer(code) {
  ...
  myPeer.on('open', (id) => {
    ...
    guestConnection.on('error', (err) => {
      showToast('Ошибка соединения с хостом.');
    });

    // Guest heartbeat: retry JOIN until we appear in players list
    if (guestHeartbeatTimer) clearInterval(guestHeartbeatTimer);
    guestHeartbeatTimer = setInterval(() => {
      if (!isHost && guestConnection && guestConnection.open) {
        const hasMe = roomState.players && roomState.players.some(p => p.id === myPlayerId);
        if (!hasMe) {
          sendJoin();
        }
      }
...
function handleHostIncomingData(conn, data) {
  ...
  if (data.type === 'JOIN') {
    ...
    // Direct immediate reply to this connection
    try { conn.send({ type: 'STATE_UPDATE', room: roomState }); } catch(e) {}
...
print("SUCCESS: WebRTC main.js rebuilt with direct handshake! Size:", len(full_code))
