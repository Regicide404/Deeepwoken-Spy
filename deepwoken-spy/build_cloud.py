...
# Fetch official Paho MQTT library
url = "https://cdnjs.cloudflare.com/ajax/libs/paho-mqtt/1.0.1/mqttws31.min.js"
req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
with urllib.request.urlopen(req) as resp:
    mqtt_code = resp.read().decode('utf-8')

game_code = r'''
// =========================================================================
// --- DEEPWOKEN SPY BULLETPROOF CENTRAL WEBSOCKET ENGINE ---
// =========================================================================
...
function updateConnectionDot(online) {
  ...
  if (dot) {
    ...
    dot.title = online ? 'Сеть подключена (0ms)' : 'Подключение...';
  }
}
...
function connectNetwork(roomCode, onConnected) {
  ...
  try {
    ...
    mqttClient = new Paho.MQTT.Client(BROKER_HOST, BROKER_PORT, BROKER_PATH, clientId);
  } catch (err) {
    console.error('MQTT create error:', err);
    return;
  }
  ...
  mqttClient.onConnectionLost = (responseObject) => {
    ...
    if (responseObject.errorCode !== 0 && currentRoomCode) {
      setTimeout(() => connectNetwork(currentRoomCode, onConnected), 2000);
    }
  };
  ...
  mqttClient.connect({
    ...
    onSuccess: () => {
      ...
      const baseTopic = 'dw_spy_net_' + currentRoomCode;
      mqttClient.subscribe(baseTopic + '/#', { qos: 0 });
    ...
    onFailure: (err) => {
      isConnected = false;
      updateConnectionDot(false);
      setTimeout(() => connectNetwork(currentRoomCode, onConnected), 1500);
    }
  });
}

function publishMessage(suffix, payload) {
  if (!mqttClient || !isConnected) return;
  try {
    const topic = 'dw_spy_net_' + currentRoomCode + '/' + suffix;
...
function broadcastState() {
  if (!isHost) return;
  publishMessage('state', { type: 'STATE_UPDATE', room: roomState });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-join-room').addEventListener('click', () => {
    ...
    connectNetwork(code, () => {
      ...
      if (guestRetryInterval) clearInterval(guestRetryInterval);
      guestRetryInterval = setInterval(() => {
...
print("SUCCESS: Bulletproof Cloud WebSocket main.js built! Size:", len(full_code))
