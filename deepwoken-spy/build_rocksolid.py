...
# Paho MQTT library
paho_url = "https://cdnjs.cloudflare.com/ajax/libs/paho-mqtt/1.0.1/mqttws31.min.js"
req = urllib.request.Request(paho_url, headers={'User-Agent': 'Mozilla/5.0'})
with urllib.request.urlopen(req) as resp:
    paho_js = resp.read().decode('utf-8')

game_js = r'''
// =========================================================================
// --- DEEPWOKEN SPY — 100% ROCK-SOLID UNIFIED CLOUD ENGINE (PORT 443 WSS) ---
// =========================================================================
...
// Networking State (Unified EMQX Cluster over Port 443)
let mqttClient = null;
let isConnected = false;
let guestJoinInterval = null;
let hostHeartbeatInterval = null;
let unreadChatCount = 0;
let isChatOpen = false;

// Unified EMQX Ports (Same Broker Cluster, zero partitioning!)
const EMQX_CONFIGS = [
  { host: 'broker.emqx.io', port: 443, path: '/mqtt', ssl: true, name: 'EMQX Cloud (Port 443 WSS)' },
  { host: 'broker.emqx.io', port: 8084, path: '/mqtt', ssl: true, name: 'EMQX Cloud (Port 8084 WSS)' }
];
...
function updateConnectionBadge(connected, brokerName) {
  const dot = document.getElementById('connection-status-dot');
  if (dot) {
    dot.className = 'status-indicator ' + (connected ? 'online' : '');
    dot.style.background = connected ? '#00ff88' : '#ff2a5f';
    dot.title = connected ? `Сеть: ${brokerName || 'В сети'}` : 'Подключение...';
  }
}
...
// =========================================================================
// --- UNIFIED CLOUD ENGINE (EMQX CLUSTER PORT 443 WSS) ---
// =========================================================================

function connectRealtimeNetwork(roomCode, onConnectSuccess) {
  ...
  // Pure unique ClientID to prevent ANY duplicate kick
  const uniqueClientId = 'dw_' + Math.random().toString(36).substring(2, 10) + '_' + Date.now().toString(36);
  const cfg = EMQX_CONFIGS[currentConfigIdx];

  try {
    mqttClient = new Paho.MQTT.Client(cfg.host, cfg.port, cfg.path, uniqueClientId);

    mqttClient.onConnectionLost = (responseObject) => {
      isConnected = false;
      updateConnectionBadge(false);
      
      // Auto-reconnect if active in room
      setTimeout(() => {
        if (currentRoomCode && (currentScreen === 'screen-lobby' || currentScreen === 'screen-game' || currentScreen === 'screen-voting' || currentScreen === 'screen-spy-guess')) {
          connectRealtimeNetwork(currentRoomCode, onConnectSuccess);
        }
      }, 1500);
    };
    ...
    mqttClient.onMessageArrived = (message) => {
      try {
        ...
        handleNetworkMessage(message.destinationName, payload);
      } catch (e) {}
    };
    ...
    mqttClient.connect({
      ...
      onSuccess: () => {
        isConnected = true;
        updateConnectionBadge(true, cfg.name);

        const wildcardTopic = `dw_spy_net_${currentRoomCode}/#`;
        mqttClient.subscribe(wildcardTopic, { qos: 0 });
      ...
      onFailure: (err) => {
        ...
        // Fallback between 443 and 8084 on the SAME EMQX cluster
        currentConfigIdx = (currentConfigIdx + 1) % EMQX_CONFIGS.length;
        setTimeout(() => {
          if (currentRoomCode) connectRealtimeNetwork(currentRoomCode, onConnectSuccess);
        }, 1000);
      }
    });
...
function publishMessage(subTopic, payload) {
  if (!mqttClient || !isConnected || !currentRoomCode) return;
  try {
    const topic = `dw_spy_net_${currentRoomCode}/${subTopic}`;
...
function broadcastState() {
  if (!isHost) return;
  publishMessage('state', { room: roomState });
}

function sendPlayerEvent(eventType, extraData) {
  const payload = Object.assign({
    type: eventType,
    playerId: myPlayerId,
    playerName: myPlayerName
  }, extraData || {});
  publishMessage('events', payload);
}
...
function sendChatMessage(text) {
  ...
  publishMessage('chat', { msg: msgObj });
}

// --- Message Router ---
function handleNetworkMessage(topic, data) {
  if (topic.endsWith('/chat') && data.msg) {
    handleIncomingChatMessage(data.msg);
    return;
  }

  if (topic.endsWith('/state') && data.room) {
  ...
  if (topic.endsWith('/events') && data) {
    if (isHost) handleHostIncomingEvent(data);
  }
}
...
function handleHostIncomingEvent(event) {
  ...
  if (event.type === 'JOIN' || event.type === 'GET_STATE') {
    ...
    if (!existing && event.type === 'JOIN') {
      ...
      broadcastSystemChatMessage('👋 ' + event.playerName + ' вошел в лобби!');
    } else if (existing) {
...
function startOnlineGameHost() {
  ...
  roomState.players.forEach((p, idx) => {
    if (spyIndices.has(idx)) {
      p.isSpy = true;
      p.word = '🕵️ ТЫ ШПИОН!';
      p.role = 'Ты не знаешь слово! Слушай игроков в Discord VC и притворяйся мирным!';
    } else {
      p.isSpy = false;
...
function updateLobbyUI(room) {
  ...
  if (isHost) {
    ...
    startBtn.textContent = (room.players.length < 3) ? `🚀 НАЧАТЬ ИГРУ (Нужно ещё ${3 - room.players.length})` : '🚀 НАЧАТЬ ИГРУ';
  ...
  if (!room.players || room.players.length === 0) {
    grid.innerHTML = '<div style="color: var(--text-muted); font-size: 0.9rem; text-align: center; width: 100%;">Ожидание игроков...</div>';
    return;
  }
...
function renderPlayingCardUI(roleObj) {
  ...
  if (roleObj.isSpy) {
    ...
    roleEl.innerHTML = `
      ...
      <span>Все мирные знают это слово. Давай осторожные ассоциации в Discord VC, чтобы шпион не догадался!</span>
    `;
  }
}
...
function renderGameRoomState(room) {
  ...
  // Decisions at the end of Circle 3+
...
// --- DOM Event Bindings & Phone Sleep Recovery ---
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-create-online').addEventListener('click', () => {
    ...
    connectRealtimeNetwork(code, () => {
      ...
      hostHeartbeatInterval = setInterval(() => {
        if (isHost && isConnected && roomState.state === 'LOBBY') {
          broadcastState();
        }
      }, 1500);
    });
  ...
  document.getElementById('btn-join-room').addEventListener('click', () => {
    ...
    document.getElementById('lobby-code-display').textContent = code;
    showScreen('screen-lobby');
    updateLobbyUI({ code: code, players: [], settings: { selectedCategories: [] } });
    showToast('Подключение к комнате ' + code + '...');

    connectRealtimeNetwork(code, () => {
      sendPlayerEvent('JOIN');

      if (guestJoinInterval) clearInterval(guestJoinInterval);
      guestJoinInterval = setInterval(() => {
        // Repeat JOIN until I am present in player list
        const meInRoom = roomState.players.some(p => p.id === myPlayerId || p.name === myPlayerName);
        if (!isHost && isConnected && !meInRoom) {
          sendPlayerEvent('JOIN');
        } else if (meInRoom && guestJoinInterval) {
          clearInterval(guestJoinInterval);
        }
      }, 800);
    });
  });
  ...
  document.getElementById('btn-pass-turn').addEventListener('click', () => {
    ...
    else sendPlayerEvent('PASS_TURN');
  });
  ...
  document.getElementById('btn-start-next-circle').addEventListener('click', () => {
    if (isHost) {
      ...
      sendPlayerEvent('START_NEXT_CIRCLE');
    }
  });
  ...
  document.getElementById('btn-trigger-voting').addEventListener('click', () => {
    if (isHost) {
      ...
      sendPlayerEvent('TRIGGER_VOTING');
    }
  });
  ...
  document.getElementById('btn-submit-vote').addEventListener('click', () => {
    ...
    if (isHost) {
      ...
      sendPlayerEvent('CAST_VOTE', { targetId: selectedVoteTargetId });
    }
  ...
  document.getElementById('btn-spy-submit-guess').addEventListener('click', () => {
    ...
    if (select && select.value) {
      ...
      else sendPlayerEvent('SPY_GUESS', { guessedWord: select.value });
    }
  });
  ...
  // Automatic Mobile Sleep / Tab Focus Recovery
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden && currentRoomCode && !isConnected) {
      connectRealtimeNetwork(currentRoomCode, () => {
        if (!isHost) sendPlayerEvent('JOIN');
      });
    }
  });
...
full_main_js = paho_js + "\n\n" + game_js

with open(main_file, "w", encoding="utf-8") as f:
    f.write(full_main_js)

print("SUCCESS: main.js written! Size:", len(full_main_js))
print("Open braces:", full_main_js.count('{'), "Close braces:", full_main_js.count('}'))
assert full_main_js.count('{') == full_main_js.count('}'), "Braces mismatch!"
