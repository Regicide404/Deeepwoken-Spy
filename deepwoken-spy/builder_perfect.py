...
game_code = r'''// =========================================================================
// --- DEEPWOKEN SPY BULLETPROOF ENGINE (HTTPS PORT 443 SYNC) ---
// =========================================================================
...
// Persistent Player ID per browser session
...
function updateConnectionDot(online) {
  ...
  if (dot) {
    ...
    dot.title = online ? 'HTTPS Сеть (0ms)' : 'Подключение...';
  }
}
...
// =========================================================================
// --- NATIVE HTTPS RELAY LOGIC (ntfy.sh PORT 443) ---
// =========================================================================

const ENDPOINT_BASE = 'https://ntfy.sh/dwspy_room_v5_';

function postNetworkMessage(payload) {
  if (!currentRoomCode) return;
  fetch(ENDPOINT_BASE + currentRoomCode, {
    method: 'POST',
...
function pollNetworkMessages() {
  if (!currentRoomCode) return;
  fetch(ENDPOINT_BASE + currentRoomCode + '/json?poll=1')
    .then(r => r.text())
    .then(text => {
...
function startNetworkSync(code) {
  ...
  pollNetworkMessages();
  pollTimer = setInterval(pollNetworkMessages, 750);
}
...
print("SUCCESS: Perfect main.js built! Size:", len(game_code))
