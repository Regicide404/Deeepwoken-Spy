...
game_code = r'''
// =========================================================================
// --- DEEPWOKEN SPY PRODUCTION ENGINE (PERFECT SYNC & DEDICATED SPY HUD) ---
// =========================================================================
...
function publishMQTT(topicSuffix, payloadObj, retained) {
  ...
  try {
    ...
    message.qos = 0;
    message.retained = Boolean(retained);
...
function broadcastState() {
  if (!isHost) return;
  publishMQTT('state', { type: 'STATE_UPDATE', room: roomState }, true);
}
...
function sendEvent(eventType, extraData) {
  ...
  publishMQTT('events', payload, false);
}
...
function handleIncomingGameEvent(event) {
  ...
  if (event.type === 'JOIN' || event.type === 'GET_STATE') {
    ...
    if (!existing && event.type === 'JOIN') {
      ...
      existing.id = event.playerId; // Sync ID if reconnected
...
function startOnlineGameHost() {
  ...
  // 1. Pick exactly ONE secret category and ONE secret item for this whole round
  ...
  // Assign roles: Innocents get the EXACT SAME chosenItem.name, Spy gets '🕵️ ВЫ ШПИОН!'
