// --- Deepwoken Spy Production Multiplayer & Game Logic (MQTT over WebSockets) ---
...
// Pass & Play (Local single-device mode)
let passPlayState = {
  playerCount: 4,
  selectedCategories: ['oaths', 'attunements', 'races', 'bosses', 'bells'],
  currentPlayerIdx: 0,
...
function playSound(type) {
  ...
  try {
    ...
    if (type === 'bell') {
      ...
      osc.frequency.exponentialRampToValueAtTime(1046.5, now + 1.2); // C6
...
function connectMQTT(roomCode, onConnectCallback) {
  ...
  try {
    ...
    showToast('Ошибка инициализации сети. Обновите страницу!');
    return;
  }
...
function sendEvent(eventType, extraData = {}) {
  publishMQTT('events', { type: eventType, playerId: myPlayerId, playerName: myPlayerName, ...extraData });
}
...
function handleIncomingChatMessage(msg) {
  ...
  item.innerHTML = `
    <div class="chat-msg-header">
      <span class="chat-msg-author ${msg.isHost ? 'host' : ''}">${msg.isHost ? '👑 ' : ''}${msg.author}</span>
...
function renderVotingUI(room) {
  ...
  room.players.forEach(p => {
    if (p.id === myPlayerId) return; // Cannot vote for self
